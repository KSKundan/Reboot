        ...
        if (requireData.destination == 'PBOX') {
            ...
            child.on('close', (code) => {
                if (pboxError == 'error') {
                    ...
                    res.status(400).send("P-Box process not restarted !!!");
                } else {
                    res.status(200).send("P-Box process restarted successfully !!!");
                }
            });
        } else {
            console.log("Missing destination type !!!");
            res.status(404).send("Missing destination type !!!");
        }
    } else {
...
app.post('/retrivenow', (req, res) => {
    ...
    if (((req.body).hasOwnProperty('type')) && ((req.body).hasOwnProperty('rboxip')) | ((req.body).hasOwnProperty('pboxip'))) {
        ...
        if ((destination == 'ALL') && ((schedulePboxIP == iPP) || (scheduleRboxIP == iPR))) {
            ...
            if ((JobDetails.Pbox_JobID != null) | (JobDetails.Rbox_JobID != null)) {
                ...
                if (JobDetails.Rbox_JobID != null) {
                    ...
                    res.status(200).send(responseFromServer + " R-Box is not scheduled  on I.P. " + iPR + " !");
                }
        ...
        res.status(404).send(" Some value is missing !!!");
    }
})
